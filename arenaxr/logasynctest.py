...
Run this code to start logging from crazyflie and sending to ArenaXR. Hit 'e' to ramp the motors and fly the drone upwards. Hit 'q' to stop the motors and land the drone.
"""
import logging
import time
...
class StateLogger:
    ...
    def _stab_log_data(self, timestamp, data, logconf):
        ...
        # land the crazyflie if it gets above 0.5 meters to avoid hitting the ceiling
        if z > 0.5:
            thrust = 30000
            while thrust:
...
if __name__ == '__main__':
    ...
    # start crazyflie logging (non blocking)
    sl = StateLogger(uri)

    # set up keyboard listener (non blocking)
    listener = keyboard.Listener(
        on_press=lambda event: on_press(event, sl))
    listener.start()

    # start arena event loop (blocking)
    scene.run_tasks()
